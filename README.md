...
> **The whole site is one file: `index.html`.** CSS and JS are inlined
> directly into it — there's nothing else to keep alongside it, and no way
> for a stylesheet or script to "go missing" if you copy or share just that
> one file.
>
> **Don't just double-click `index.html` from your file browser.** Opening
> a file directly (a `file://` URL) makes browsers block the page from
> loading its own data files — you'll get a page with no leaderboard/roster
> data even though it looks otherwise fine. Either:
> - run a local server in this folder (`python3 -m http.server`, then open
>   `http://localhost:8000`), or
> - deploy it to GitHub Pages (see **Deploying** below) — this is the real
>   way you're meant to run it.
...
Pages. Rosters, points/hour trends, "gap to your leaderboard neighbors"
cards, points-to-rank-up, and a live current-battle view, all built on the
public [BIG Games API](https://github.com/BIG-Games-LLC/ps99-public-api-docs).
...
built-in `fetch`, and `index.html` is a single self-contained file (Chart.js
is pulled from a CDN `<script>` tag; everything else is inline).

## Local preview

Any static file server works — do **not** just open the file directly:
